...
class ArtNameProvider(private val appLocaleManager: AppLocaleManager) {
    private var nameMap: Map<String, ArtTranslation> = emptyMap()
    ...
    suspend fun load() {
        ...
        try {
            val bytes = Res.readBytes(ART_JSON_PATH)
            val jsonString = bytes.decodeToString()
            val items = json.decodeFromString<List<ArtTranslation>>(jsonString)
    ...
    @Serializable
    private data class ArtTranslation(
