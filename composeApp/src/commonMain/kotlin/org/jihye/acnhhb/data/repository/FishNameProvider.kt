...
class FishNameProvider(private val appLocaleManager: AppLocaleManager) {
    private var nameMap: Map<String, FishTranslation> = emptyMap()
    ...
    suspend fun load() {
        ...
        try {
            val bytes = Res.readBytes(FISH_JSON_PATH)
            val jsonString = bytes.decodeToString()
            val items = json.decodeFromString<List<FishTranslation>>(jsonString)
    ...
    @Serializable
    private data class FishTranslation(
    ...
    companion object {
        private const val FISH_JSON_PATH = "files/translate/fish.json"
