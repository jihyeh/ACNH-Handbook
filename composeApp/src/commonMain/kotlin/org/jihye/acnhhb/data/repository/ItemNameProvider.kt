...
class ItemNameProvider(private val appLocaleManager: AppLocaleManager) {
    private var nameMap: Map<String, ItemTranslation> = emptyMap()
    ...
    suspend fun load() {
        ...
        try {
            val bytes = Res.readBytes(ITEM_JSON_PATH)
            val jsonString = bytes.decodeToString()
            val items = json.decodeFromString<List<ItemTranslation>>(jsonString)
