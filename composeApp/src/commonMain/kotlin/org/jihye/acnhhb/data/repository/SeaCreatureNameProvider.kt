...
class SeaCreatureNameProvider(private val appLocaleManager: AppLocaleManager) {
    private var nameMap: Map<String, SeaCreatureTranslation> = emptyMap()
    ...
    suspend fun load() {
        ...
        try {
            val bytes = Res.readBytes(SEA_CREATURES_JSON_PATH)
            val jsonString = bytes.decodeToString()
            val items = json.decodeFromString<List<SeaCreatureTranslation>>(jsonString)
    ...
    @Serializable
    private data class SeaCreatureTranslation(
    ...
    companion object {
        private const val SEA_CREATURES_JSON_PATH = "files/translate/sea_creatures.json"
