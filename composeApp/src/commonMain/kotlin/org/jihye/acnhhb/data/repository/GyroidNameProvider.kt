...
class GyroidNameProvider(private val appLocaleManager: AppLocaleManager) {
    private var nameMap: Map<String, GyroidTranslation> = emptyMap()
    ...
    suspend fun load() {
        ...
        try {
            val bytes = Res.readBytes(GYROIDS_JSON_PATH)
            val jsonString = bytes.decodeToString()
            val items = json.decodeFromString<List<GyroidTranslation>>(jsonString)
    ...
    @Serializable
    private data class GyroidTranslation(
    ...
    companion object {
        private const val GYROIDS_JSON_PATH = "files/translate/gyroids.json"
